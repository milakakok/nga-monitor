...
# ====================== 只改这里 ======================
WATCH_UIDS = ["150058", "26529713", "66662897", "557398"]  # 监控的用户UID
    ...
    for uid in WATCH_UIDS:
        try:
            ...
            if uid not in last_tid or latest_tid > last_tid[uid]:
                ...
                print(f"用户{uid}有新发言: {latest_tid}")
            else:
                print(f"用户{uid}无新发言")
        except Exception as e:
            print(f"❌ 处理用户{uid}出错: {e}")
